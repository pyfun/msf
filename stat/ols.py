...
from numpy import log, pi, sqrt, square, diagonal, c_, ones, zeros, tile, dot, diff
...
class ols:
    ...
    def nw(self,nlags):
        """
        Calculate newey-west corrected covariance matrix
        """
        u =self.e.reshape(self.nobs,1)*self.x
        S = dot(u.T,u)
        for lag in xrange(1,nlags+1):
            rho = dot(u[0:self.nobs-lag,:].T,u[lag:self.nobs,:])
            rho = rho + rho.T
            wt = 1 - lag/(nlags+1)
            S = S + wt * rho
        V = self.inv_xx
        V = dot(dot(V,S),V)
        self.nwse = sqrt(diagonal(V))  # coef. standard errors
